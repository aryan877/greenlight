...
export const buildMcpServer = ({
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const baseArtifact = artifacts.resolveArtifact(
      ...
      const contentArtifact = await artifacts.importJson({
        projectId: selection.project_id,
        kind: "content_package",
        value: revised,
        provenance: {
      ...
      return result({
        ...
        project: store.setProjectStage(selection.project_id, "packaged"),
      });
    },
  );
